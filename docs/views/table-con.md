...
```html
  <template>
    ...
    <s-table-con
      ...
      >
      <template v-slot:action="scope">
        <s-button v-if="editRowsArray.indexOf(scope.$index) === -1" size="mini" type="primary" @click="setEdit(scope.$index)">编辑</s-button>
        <template v-else>
          <s-button size="mini" type="primary" @click="save(scope.$index)">保存</s-button>
          <s-button size="mini" @click="setEdit(scope.$index, false)">取消</s-button>
        </template>
      </template>
    </s-table-con>
  </template>
